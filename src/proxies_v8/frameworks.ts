...
frameworksApi.use('/*', jsonParser, async (req, res) => {
  try {
    ...
    logDebug(`Framework API call: Users Roles are... ${userRoleData} : rootOrgId: ${userRootOrgId}`)
...
const sendFrameworkAPIRequest = async (req: express.Request, res: express.Response, url: string, userRootOrgId: string) => {
  try {
    logDebug(`sendFrameworkAPIRequest the url is... ${CONSTANTS.KONG_API_BASE}${url}}`)
