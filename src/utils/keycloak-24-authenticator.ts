...
export class Keycloak24Authenticator {
  private multiTenantKeycloak = new Map<string, keycloakConnect>()

  constructor(sessionConfig: expressSession.SessionOptions) {
    if (CONSTANTS.MULTI_TENANT_KEYCLOAK_24) {
      CONSTANTS.MULTI_TENANT_KEYCLOAK_24.split(';').forEach((v: string) => {
  ...
  authenticated = (reqObj: any, next: any) => {
    logInfo('Step 3: Keycloak24 authenticated function', '------', new Date().toString())
    ...
    async.series(postLoginRequest, (err: any) =>  {
      if (err) {
        ...
        logInfo(`${process.pid}: Keycloak24 User authenticated`, '------', new Date().toString())
        next(null, 'loggedin')
      }
    })
  }
  ...
  deauthenticatedNew = (reqObj: any) => {
    ...
    logInfo(`${process.pid}: Keycloak 24 User Deauthenticated New`)
  }
  ...
  deauthenticated = (reqObj: any) => {
    ...
    delete reqObj.session.keycloakClientSecret
    reqObj.session.destroy()
    logInfo(`${process.pid}: Keycloak24 User Deauthenticated`)
  }
