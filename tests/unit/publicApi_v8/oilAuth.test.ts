...
jest.mock('../../../src/publicApi_v8/ssoUserHelper', () => ({
...
import { oilAuth } from '../../../src/publicApi_v8/oilAuth'
import { createUserWithMailId, fetchUserByEmailId, updateKeycloakSession } from '../../../src/publicApi_v8/ssoUserHelper'
...
describe('oilAuth GET /auth', () => {
  it('stores a state token in redis and redirects to the OIL authorization url', async () => {
    mockedRedis.set.mockResolvedValue('OK')
    const res = await supertest(buildApp()).get('/auth')
    expect(res.status).toBe(302)
    expect(res.header.location).toContain(CONSTANTS.OIL_AUTH_URL)
    expect(mockedRedis.set).toHaveBeenCalledWith(expect.stringContaining('oil_auth_state:'), 'VALID', 'EX', 300)
  })
})

describe('oilAuth GET /login/callback', () => {
  ...
  it('redirects new users to /public/welcome after registering them', async () => {
    ...
    mockedAxios.mockResolvedValueOnce({ data: { givenName: 'Jane', mail: 'jane@oil.com', mobilePhone: '999', surname: 'Doe' } })
    ...
    expect(mockedCreateUserWithMailId).toHaveBeenCalledWith('jane@oil.com', 'Jane', 'Doe', '999', 'oilIndia')
  })

  it('redirects to logout with an error when OIL does not return a mail', async () => {
    mockedRedis.get.mockResolvedValue('VALID')
    mockedAxios.mockResolvedValueOnce({ data: { access_token: 'kc-token' } })
    mockedJwtDecode.mockReturnValue({ oid: 'oid-1' })
