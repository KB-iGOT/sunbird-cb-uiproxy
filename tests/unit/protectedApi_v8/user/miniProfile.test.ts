jest.mock('axios', () => ({ get: jest.fn() }))

import axios from 'axios'
import express from 'express'
import supertest from 'supertest'
import { CONSTANTS } from '../../../../src/utils/env'
import { userMiniProfile } from '../../../../src/protectedApi_v8/user/miniProfile'

const mockedAxios = axios as unknown as { get: jest.Mock }

function buildApp() {
  const app = express()
  app.use(userMiniProfile)
  return app
}

describe('userMiniProfile', () => {
  it('fetches the mini profile for the given userId with org headers', async () => {
    mockedAxios.get.mockResolvedValue({ data: { name: 'Jane' } })
    const res = await supertest(buildApp()).get('/user-1').set('rootOrg', 'igot').set('org', 'dopt')

    expect(res.status).toBe(200)
    expect(res.body).toEqual({ name: 'Jane' })
    expect(mockedAxios.get).toHaveBeenCalledWith(
      `${CONSTANTS.NODE_API_BASE}/userprofiles/pathfinders/viewprofile`,
      expect.objectContaining({
        headers: expect.objectContaining({ org: 'dopt', rootOrg: 'igot', user_id: 'user-1', wid: 'user-1' }),
      })
    )
  })

  it('falls back to a generic 500 error when the failure has no response', async () => {
    mockedAxios.get.mockRejectedValue(new Error('down'))
    const res = await supertest(buildApp()).get('/user-1')
    expect(res.status).toBe(500)
    expect(res.body).toEqual({ error: 'Failed due to unknown reason' })
  })
})
