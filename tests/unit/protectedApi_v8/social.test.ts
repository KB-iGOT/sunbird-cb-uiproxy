...
jest.mock('axios')
jest.mock('form-data', () => jest.fn().mockImplementation(() => ({ append: mockFormAppend, submit: mockFormSubmit })))
...
import { socialApi } from '../../../src/protectedApi_v8/social'
...
const ADMIN_BODY = { a: 1, adminId: 'user-1', org: 'o1', rootOrg: 'r1' }
...
const ROUTES: IRouteCase[] = [
  ...
  routeCase('/post/autocomplete', '/post/autocomplete', REQ_BODY, { ...axiosRequestConfig, headers: ORG_HEADERS }),
  ...
  routeCase('/moderator/moderatepost', '/moderator/moderatepost', { a: 1, moderatorId: 'user-1', org: 'o1', rootOrg: 'r1' }, SOCIAL_CONFIG),
  routeCase('/moderator/timeline', '/moderator/timeline', USER_BODY, SOCIAL_CONFIG),
  routeCase('/admin/timeline', '/admin/timeline', USER_BODY, SOCIAL_CONFIG),
  routeCase('/admin/deletePost', '/admin/deletepost', ADMIN_BODY, axiosRequestConfig, 'post', 'delete'),
  routeCase('/admin/reactivatePost', '/admin/reactivatepost', ADMIN_BODY, SOCIAL_CONFIG),
  routeCase('/viewForum', '/forum/viewforum', USER_BODY, SOCIAL_CONFIG),
  routeCase('/forum/forumtimeline', '/forum/forumtimeline', USER_BODY, SOCIAL_CONFIG),
  routeCase('/post/activity/create', '/useractivity/create', USER_BODY, axiosRequestConfig),
  routeCase('/createForum', '/forum/createforum', { a: 1, forumCreator: 'user-1', org: 'o1', rootOrg: 'r1' }, axiosRequestConfig),
  routeCase('/editForum', '/forum/editforum', { a: 1, forumEditor: 'user-1', org: 'o1', rootOrg: 'r1' }, axiosRequestConfig),
  ...
  routeCase('/post/search', '/search/searchv1', USER_BODY, axiosRequestConfig),
...
describe.each(ROUTES)('socialApi $verb $path', (route) => {
  ...
  it('lets org/rootOrg headers override body fields', async () => {
    ...
    if (route.path === '/post/autocomplete') {
      expect(data).toEqual({ org: 'x', rootOrg: 'y' })
    } else {
...
describe('socialApi user id resolution', () => {
  it('prefers the wid header over the token subject', async () => {
    await supertest(buildApp()).post('/moderator/timeline').set({ ...ORG_HEADERS, wid: 'wid-1' }).send({})
    expect(upstreamCalls()[0].data).toEqual({ org: 'o1', rootOrg: 'r1', userId: 'wid-1' })
  })

  it('sends an undefined user id when there is no token (key is dropped in JSON)', async () => {
    await supertest(buildApp(false)).post('/createForum').set(ORG_HEADERS).send({})
    const data = upstreamCalls()[0].data
    expect(Object.keys(data)).toEqual(['forumCreator', 'org', 'rootOrg'])
    expect(data.forumCreator).toBeUndefined()
  })

  it('POST /post/timelineV2 prefers the wid query parameter', async () => {
    await supertest(buildApp()).post('/post/timelineV2?wid=q-1').set({ ...ORG_HEADERS, wid: 'wid-1' }).send({})
    expect(upstreamCalls()[0].data).toEqual({ org: 'o1', rootOrg: 'r1', userId: 'q-1' })
  })

  it('body userId is overridden by the resolved user id', async () => {
    await supertest(buildApp()).post('/post/search').set(ORG_HEADERS).send({ userId: 'spoof' })
    expect(upstreamCalls()[0].data).toEqual({ org: 'o1', rootOrg: 'r1', userId: 'user-1' })
  })
})

describe('socialApi POST /post/upload/:contentId', () => {
  const uploadUrl = `${CONSTANTS.CONTENT_API_BASE}/contentv3/upload-live/r1%2Fo1%2FPublic%2Fc1%2Fartifacts`
  const upload = (headers: Record<string, string> = ORG_HEADERS) =>
    supertest(buildApp()).post('/post/upload/c1').set(headers).attach('content', Buffer.from('hello'), 'a.png')

  function respondWith(err: unknown, statusCode: number, payload?: object) {
    mockFormSubmit.mockImplementation((_url: string, cb: (e: unknown, r: EventEmitter) => void) => {
  ...
  it('returns 400 when org headers are missing', async () => {
    const res = await upload({ org: 'o1' })
    expect(res.status).toBe(400)
    expect(res.text).toBe('ERROR_NO_ORG_DATA')
    expect(mockFormSubmit).not.toHaveBeenCalled()
  })

  it('returns 500 with the general error when no file is attached', async () => {
    const res = await supertest(buildApp()).post('/post/upload/c1').set(ORG_HEADERS).send({})
    expect(res.status).toBe(500)
    expect(res.body).toEqual(GENERAL_ERROR)
  })

  it('submits the file to the content upload endpoint and relays the JSON reply', async () => {
    respondWith(null, 201, { artifactUrl: 'u' })
    const res = await upload()
    expect(res.status).toBe(200)
    expect(res.body).toEqual({ artifactUrl: 'u' })
    expect(mockFormSubmit.mock.calls[0][0]).toBe(uploadUrl)
    expect(mockFormAppend).toHaveBeenCalledWith('content', Buffer.from('hello'), {
      contentType: 'image/png',
      filename: 'a.png',
    })
  })

  it('sends the submit error message on a non-2xx reply', async () => {
  ...
  it('sends the general error on a non-2xx reply without error', async () => {
    respondWith(null, 502)
    const res = await upload()
    expect(res.status).toBe(200)
    expect(res.body).toEqual(GENERAL_ERROR)
  })
